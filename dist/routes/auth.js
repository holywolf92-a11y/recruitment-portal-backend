...
const portalAccessService_1 = require("../services/portalAccessService");
...
router.get('/portal-requirements', auth_1.authenticate, async (req, res) => {
    try {
        const user = req.user;
        if (!user)
            return res.status(401).json({ error: 'Unauthorized' });
        if (user.role !== 'employer')
            return res.status(403).json({ error: 'Only employer accounts can access requirements' });
        const db = (0, database_1.supabaseAdminClient)();
...
router.post('/portal-requirements', auth_1.authenticate, async (req, res) => {
    try {
        const user = req.user;
        if (!user)
            return res.status(401).json({ error: 'Unauthorized' });
        if (user.role !== 'employer')
            return res.status(403).json({ error: 'Only employer accounts can post requirements' });
        const db = (0, database_1.supabaseAdminClient)();
...
router.post('/users', auth_1.authenticate, async (req, res) => {
    try {
        ...
        if (['candidate', 'partner', 'employer'].includes(normalizedRole)) {
            credentialDelivery = await (0, portalAccessService_1.dispatchPortalAccessLink)({
                name: createName,
                email: createEmail,
                phone: createPhone,
                role: normalizedRole,
...
router.post('/users/:userId/send-credentials', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const credentialDelivery = await (0, portalAccessService_1.dispatchPortalAccessLink)({
