...
router.get('/', async (req, res) => {
    try {
        ...
        if (search) {
            query = query.or(`company_name.ilike.%${search}%,contact_name.ilike.%${search}%,email.ilike.%${search}%,country.ilike.%${search}%,professions.ilike.%${search}%`);
        }
        if (status) {
...
router.post('/', async (req, res) => {
    try {
        const db = (0, database_1.supabaseAdminClient)();
        const { company_name, contact_name, email, phone_number, country, city, professions, quantity, salary_range, duty_hours, contract_duration, benefits_included, comments, status, } = req.body;
...
router.put('/:id', async (req, res) => {
    try {
        ...
        if (req.body.company_name !== undefined)
            updates.company_name = req.body.company_name;
        if (req.body.contact_name !== undefined)
            updates.contact_name = req.body.contact_name;
        if (req.body.professions !== undefined)
            updates.professions = req.body.professions;
        if (req.body.quantity !== undefined)
            updates.quantity = req.body.quantity;
        if (req.body.country !== undefined)
            updates.country = req.body.country;
        if (req.body.salary_range !== undefined)
            updates.salary_range = req.body.salary_range;
        const { data, error } = await db
            .from('employer_leads')
            .update(updates)
            .eq('id', id)
