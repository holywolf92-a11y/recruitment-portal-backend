...
exports.getFalishaKnowledgeBase = getFalishaKnowledgeBase;
...
function getFalishaKnowledgeBase() {
    return FALISHA_KNOWLEDGE_BASE;
}
function retrieveFalishaKnowledge(params) {
    ...
    const scored = FALISHA_KNOWLEDGE_BASE.articles
        ...
        if (intentId && article.intents.includes(intentId)) {
            score += 4;
...
function assessFalishaKnowledgeSupport(params) {
    ...
    if (!supportedIntentIds.has(intentId)) {
        return {
    ...
    const matchedIntentArticles = intentId
        ? articles.filter((article) => article.intents.includes(intentId))
        : [];
...
function buildFalishaKnowledgeContext(params) {
    const articles = retrieveFalishaKnowledge(params);
    if (articles.length === 0) {
        return '';
    }
