...
import {
  ...
  getOriginalCvSignedUrlForCandidate,
 ...
 */
export async function getCandidateCvUrlController(req: Request, res: Response) {
  try {
    const { candidateId } = req.params;
    if (!candidateId) {
      return res.status(400).json({ error: 'Candidate ID is required' });
    }
    const signedUrl = await getOriginalCvSignedUrlForCandidate(candidateId);
    if (!signedUrl) {
