...
import { recordOutboundMessage, isWithin24HourWindow } from '../services/whatsappInboxService';
import { patchBotData } from '../services/whatsappBotStateService';
import { sendButtons, WaButton } from '../services/whatsappInteractiveService';

const logger = createLogger('WhatsAppFlowReminderWorker');

const CHECK_INTERVAL_MS = Number(
  process.env.WHATSAPP_FLOW_REMINDER_CHECK_INTERVAL_MS
    || process.env.WHATSAPP_CANDIDATE_REMINDER_CHECK_INTERVAL_MS
    || 15 * 60 * 1000,
);
const REMINDER_GAP_MS = Number(
  process.env.WHATSAPP_FLOW_REMINDER_GAP_MS
    || process.env.WHATSAPP_CANDIDATE_REMINDER_IDLE_MS
    || 24 * 60 * 60 * 1000,
);
const MAX_REMINDERS = Number(
  process.env.WHATSAPP_FLOW_REMINDER_MAX_COUNT
    || process.env.WHATSAPP_CANDIDATE_REMINDER_MAX_COUNT
    || 3,
);
const START_DELAY_MS = Number(
  process.env.WHATSAPP_FLOW_REMINDER_START_DELAY_MS
    || process.env.WHATSAPP_CANDIDATE_REMINDER_START_DELAY_MS
    || 2 * 60 * 1000,
);

const MENU_HUMAN_BUTTONS: WaButton[] = [
  { id: 'main_menu', title: 'Main Menu' },
  { id: 'talk_human', title: 'Talk to Human' },
];

const CANDIDATE_CV_BUTTONS: WaButton[] = [
  ...
  candidate_intake: {
    ...
    steps: {
      basic_details: {
        ...
        buildBody: (botData: Record<string, any>) => {
          const required = [
            ['name', 'Full Name'],
...
type SupportedFlow = keyof typeof FLOW_CONFIGS;
type ReminderConversation = {
  id: string;
  phone_number: string;
  bot_flow: SupportedFlow;
  bot_step: string | null;
  bot_data: Record<string, any> | null;
  last_inbound_at: string | null;
  reply_mode: 'ai' | 'human' | null;
};

type ReminderStepConfig = {
  header: string;
  buttons: readonly WaButton[];
  buildBody: (botData: Record<string, any>) => string;
};

let reminderTimer: NodeJS.Timeout | null = null;
let reminderRunInFlight = false;

function getReminderMetadata(conversation: ReminderConversation) {
...
async function fetchIncompleteConversations(): Promise<ReminderConversation[]> {
  ...
  return (data ?? []) as ReminderConversation[];
}

async function sendReminder(conversation: ReminderConversation, phoneNumberId: string, accessToken: string): Promise<void> {
  ...
  const stepConfig = flowConfig.steps[step as keyof typeof flowConfig.steps] as ReminderStepConfig | undefined;
  if (!stepConfig) {
    return;
  }
  ...
  await patchBotData(conversation.phone_number, {
...
async function runReminderPass(): Promise<void> {
    ...
    const conversations = await fetchIncompleteConversations();
     ...
     for (const conversation of conversations) {
      ...
      if (!baselineAt || now - baselineAt < REMINDER_GAP_MS) {
       ...
       const withinWindow = await isWithin24HourWindow(conversation.id).catch((error) => {
  ...
  export function startWhatsAppCandidateReminderWorker(): void {
